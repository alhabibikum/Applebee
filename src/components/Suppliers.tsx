  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-2 sm:space-y-0 animate-slide-up">
        <h2 className="text-xl sm:text-2xl font-bold text-teal-900">Supplier Management</h2>
      ...
      <div className="block sm:hidden">
        <div className="grid grid-cols-3 gap-1 bg-teal-100 p-1 rounded-lg">
          {tabs.map((tab) => (
            <button
              key={tab.id}
          ...
          <nav className="-mb-px flex space-x-8">
            {tabs.map((tab) => (
              ...
              >
                ...
                {tab.name}
                {tab.count !== null && (
                  <span className="ml-2 bg-teal-100 text-teal-900 py-0.5 px-2.5 rounded-full text-xs">
                    {tab.count}
                  </span>
                )}
              </button>
    ...
    <div className="space-y-4">
      ...
      <div className="block sm:hidden space-y-3">
        {suppliers.map((supplier: any) => (
          <div key={supplier._id} className="bg-white rounded-lg shadow p-4 border border-teal-200 card-hover">
            <div className="flex justify-between items-start mb-3">
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-teal-900 text-sm truncate">{supplier.name}</h3>
                <p className="text-xs text-teal-600 truncate">{supplier.contactPerson}</p>
              </div>
              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                supplier.isActive ? "bg-green-100 text-green-800" : "bg-teal-100 text-gray-800"
              }`}>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {suppliers.map((supplier: any) => (
                <tr key={supplier._id} className="hover:bg-teal-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <div className="text-sm font-medium text-teal-900">{supplier.name}</div>
                      <div className="text-sm text-teal-500">{supplier.contactPerson}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-teal-900">{supplier.phone}</div>
                    <div className="text-sm text-teal-500">{supplier.email}</div>
                  </td>
      ...
      {suppliers.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-teal-500">No suppliers found.</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="block sm:hidden space-y-3">
        {purchases.map((purchase: any) => (
          <div key={purchase._id} className="bg-white rounded-lg shadow p-4 border border-teal-200 card-hover">
            <div className="flex justify-between items-start mb-3">
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-teal-900 text-sm truncate">{purchase.purchaseNumber}</h3>
                <p className="text-xs text-teal-600 truncate">{purchase.supplierName}</p>
              </div>
              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
            ...
            <div className="grid grid-cols-2 gap-4 mb-3 text-sm">
              <div>
                <p className="text-teal-500">Items</p>
                <p className="font-medium">{purchase.items.length} items</p>
              </div>
              <div>
                <p className="text-teal-500">Total</p>
                <p className="font-semibold text-amber-600">৳{purchase.total.toLocaleString('en-BD')}</p>
              </div>
              <div className="col-span-2">
                <p className="text-teal-500">Date</p>
      ...
      <div className="hidden sm:block bg-white rounded-lg shadow overflow-hidden border border-teal-200">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {purchases.map((purchase: any) => (
                <tr key={purchase._id} className="hover:bg-teal-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-teal-900">
      ...
      {purchases.length === 0 && (
        <div className="text-center py-12">
          <div className="text-4xl mb-4 opacity-50">📋</div>
          <p className="text-teal-500">No purchase orders found.</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-md max-h-screen overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-teal-700 mb-1">Contact Person</label>
            <input
              type="text"
              required
          ...
          {supplier && (
            <div className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <label htmlFor="isActive" className="text-sm font-medium text-teal-700">
                Active
              </label>
            </div>
          )}
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-2xl max-h-screen overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-teal-700 mb-1">Supplier</label>
            <select
              required
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="block text-sm font-medium text-teal-700">Items</label>
              <button
                type="button"
                onClick={addItem}
            ...
            <div className="space-y-3">
              {formData.items.map((item, index) => (
                <div key={index} className="border border-teal-200 rounded-lg p-3">
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                    ...
                    <div className="flex space-x-2">
                      <input
                        ...
                        className="flex-1 px-3 py-2 border border-teal-300 rounded-lg focus:ring-2 focus:ring-amber-400 focus:border-transparent text-sm"
                      />
                      {formData.items.length > 1 && (
                        <button
                          type="button"
                          onClick={() => removeItem(index)}
                          className="px-2 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 text-xs"
                        >
                          ×
                        </button>
                      )}
          ...
          <div>
            <label className="block text-sm font-medium text-teal-700 mb-1">Tax (%)</label>
            <input
              type="number"
              min="0"
              step="0.01"
